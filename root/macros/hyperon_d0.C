...
void hyperon_d0(TList* right,TList* wrong, TList* result,hyperon& xi)
{
        Int_t nbins = 40;
        Double_t var_max = 1.01;
        Double_t var_min = 0.01;

	TH1F* h600 = new TH1F("significance_vs_hyperon_d0","significance_vs_hyperon_d0",nbins,var_min,var_max);
...
//		if (fabs(xi._1_d01c)>=0.2)         continue;
		if (xi._1_nsvx<0.5)                continue;						   

		for (int i=0; i<nbins; i++) { 
			if (fabs(xi._1_d01c)<delta*(double)(i)) { 

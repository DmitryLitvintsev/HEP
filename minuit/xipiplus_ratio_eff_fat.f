C
C unbinned likelihood fit  of Xi(1530) and Xi(1860) taking into 
C account relative efficiency Gamma=17
C
      PROGRAM MAIN
      IMPLICIT NONE 
...
C*   Open data files
C*
      open(unit=5,file='xicst_ratio_eff.data',status='old')
      open(unit=21,file='fcn_xicst_ratio_jet20_eff_fat.dat',
     &     status='unknown',form='formatted')
      open(unit=22,file='func_xicst_ratio_jet20_eff_fat.dat',
     &     status='unknown',form='formatted')
      open(unit=23,file='emat_xicst_ratio_jet20_eff_fat.dat',
     &     status='unknown',form='formatted')
      open(unit=24,file='ul_xicst_ratio_jet20_eff_fat.dat',
     &     status='unknown',form='formatted')
      ...
      DOUBLE PRECISION GCM,GCS,GCG,GCE,CONSTR,GCG1
      INTEGER NPARM
      PARAMETER(NPARM=14)
...
C* 
      ...
      DO I=1,NSTEPS
        VAR = LEFT + (DFLOAT(I)-0.5D0)*STEP
        INTEGRAL1 = INTEGRAL1+FOLDER(VAR,U(13),U(11),U(12))*STEP
      ENDDO
      
      
      SUM=U(1)*(INTEGRAL+U(10)*U(14)*INTEGRAL1)+
...
C
      GCG1   = (U(13)-17.D-3)/0.3D-3
      GCG    = (U(3)-9.1D-3)/0.5D-3
      GCS    = (U(11)-SIG)/DSIG
      GCM    = (U(12)-MASS)/2.0D-3
      GCE    = (U(14)-EFF)/DEFF

      CONSTR = 0.5*GCG*GCG+0.5*GCS*GCS+0.5*GCM*GCM+0.5*GCE*GCE+
     &     0.5*GCG1*GCG1
...
C*
         DO I=1,NPARM
          ...
          ENDIF
          IF (I.EQ.10) THEN
            VAL   = VAL   / INTEGRAL * INTEGRAL1
            ERROR = ERROR / INTEGRAL * INTEGRAL1
          ENDIF
...
C*
 ...
 1004     FORMAT( G20.10 )
       ...
       WRITE(24,1005  )I90,I95,
     &       1.D0/1000.D0*DFLOAT(I90-1)/INTEGRAL*INTEGRAL1,
     &       1.D0/1000.D0*DFLOAT(I95-1)/INTEGRAL*INTEGRAL1
 1005  FORMAT(4X,2I5,2G20.10)
...
C
      ...
      FUNCTION FOLDER(OBM,GAM,SIG,TRM)
      IMPLICIT NONE 
      COMPLEX*16 Z,WZ,WWERF
      DOUBLE PRECISION X,Y,FOLDER,OBM,GAM,SIG,TRM
      DOUBLE PRECISION ARG,SIGMA,DSIGMA,SIGAM
      ...
      WZ = WWERF(Z)
      FOLDER = DREAL(WZ)
      FOLDER = FOLDER / (2.50662827D0 * SIG )
      RETURN
      END
...
C*
      ...
      DOUBLE PRECISION BWG,FOLD,DSQRT,GAUSS,FOLD1,EFF,FOLDER
      DIMENSION U(*)
...
C
      ...
      DATA PAR  / 0.9019692650D-03/
      DATA EPAR / 0.7641573253D-04/

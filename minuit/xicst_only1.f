...
C
     ...
     &            'A0','A1','A2','A3' /
      DATA VSTRT/  1000.D0  , 3.5D-3     ,  9.1D-3 , 1.5318D0, 
...
C      OPEN(UNIT=80,FILE='jet20_data_points_rs.dat')
C      OPEN(UNIT=80,FILE='data_points_unique_rs.dat')
C      OPEN(UNIT=80,FILE='data_points_rs_deltaR.dat')
...
C      open(unit=5,file='xicst_only.data',status='old')
      open(unit=21,file='fcn_xicst_only3.dat',
     &     status='unknown',form='formatted')
      open(unit=22,file='func_xicst_only3.dat',
     &     status='unknown',form='formatted')
      open(unit=23,file='emat_xicst_only3.dat',
     &     status='unknown',form='formatted')
...
C
      ...
      GCS   = (U(2)-0.3501498240D-02)/ 0.5707087201D-03
C      GCG = 0.D0
...
C          FUN= BWG2(XDATA(I),EXDATA(I),U)
        ...
        ENDIF
      END DO
      SUM = SUM + 0.5*GCG*GCG + 0.5*GCS*GCS
      F=SUM*2.D0
      IF (IFLAG.EQ.3) THEN
        WRITE(21,1000  )F
 1000   FORMAT( G20.10 )

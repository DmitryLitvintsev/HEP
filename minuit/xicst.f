...
C
     ...
     &            'A0','A1','A2','A3' /
      DATA VSTRT/   24.D+04  , 3.2D-3     ,  9.1D-3 , 1.5318D0, 
     &             0.55732  , 3.860200D+06, -9.578600D+06,
     &             15.234000D+07, -9.266200D+06 /
      DATA STP  /   0.001 ,    0.001 ,  0.001,   6*0.001 /


      CALL HLIMIT(300000)
      CALL HROPEN(20,'blah','xist.hist','N',1024,ISTAT)
      CALL HBOOK1(1,'mass',280,1.4,2.8,0.0)


      NDATA=1
      OPEN(UNIT=80,FILE='data_points_unique_rs_xi_mass.dat')
C      OPEN(UNIT=80,FILE='data_points_unique_rs.dat')
 50   READ(80,*,END=500)NUMBER,RUN,EVENT,
     &     XDATA(NDATA),EXDATA(NDATA),EXTRAVAR
C     &     XDATA(NDATA),EXDATA(NDATA)
...
C      open(unit=5,file='xicst_only.data',status='old')
      open(unit=21,file='fcn_xicst.dat',
     &     status='unknown',form='formatted')
      open(unit=22,file='fcn_xicst.dat',
     &     status='unknown',form='formatted')
      open(unit=23,file='emat_xicst.dat',
     &     status='unknown',form='formatted')
...
C
      ARGLIS(1) = 0.
      CALL MNEXCM(FCN,'SHOW PAR', ARGLIS ,1,IERFLG,0) 
      CALL MNEXCM(FCN,'MIGRAD', ARGLIS ,0,IERFLG,0) 
C
C RELEASE SIGMA 
C
      ARGLIS(1) = 2. 
      CALL MNEXCM(FCN,'RELEASE ', ARGLIS ,1,IERFLG,0) 

...
C
      ...
      CALL HBOOK1(2,'LH',2000.0,-1000.0,1000.0,0.0)
...
C      I = 1 
      DO I=1,NDATA
        CALL HF1(1,XDATA(I),1.0)
      ENDDO

C*
C*   Open data files
C*
      open(unit=5,file='xicst2_g2.data',status='old')
      open(unit=21,file='xicst2_g2_emat_tof_pt.dat',
     &     status='unknown',form='formatted')
      open(unit=22,file='xicst2_g2_fcn_tof_pt.dat',
     &     status='unknown',form='formatted')
      open(unit=23,file='xicst2_g2_par_tof_pt.dat',
     &     status='unknown',form='formatted')
C      open(unit=6,file='xicst2_g2_tof_pt.out',
...
C      CALL HRPUT(0,'tst.hist','N')
      ...
      DOUBLE PRECISION GCM,GCS1,GCS2,GCR,GCG
      DOUBLE PRECISION LHSUM,LHRSUM,LH
      DIMENSION LH(2001)
      INTEGER IFLAG,NPAR
      INTEGER NPARM,I,I90,I95,HAS90,HAS95
      PARAMETER(NPARM=15)
...
C
      ...
      GCS1   = (U(10)-0.0039D0)/8.D-5
      GCS2   = (U(14)-0.0096D0)/0.0002D0
      GCR    = (U(15)-1.94)/0.16
      DO I=1,NDATA
        ...
        ENDIF
      END DO
      SUM = SUM + 0.5*GCS1*GCS1 + 0.5*GCM*GCM + 
     &     0.5*GCS2*GCS2+0.5*GCR*GCR
      F=SUM*2.D0
      IF (IFLAG.EQ.3) THEN
C        WRITE(21,1000  )F
c 1000   FORMAT( G20.10 )
        ...
        DO I=1,NPARM
          WRITE(21,1002)(EMAT(I,J),J=1,NPARM)
 1002     FORMAT(15x,15G20.10)
        ENDDO
        
        DO I=1,NPARM
          CALL MNERRS(-I,EPLUS,EMINUS,EPARAB,GLOBCC)
          WRITE(23,1003  )I,U(I),EPARAB,EPLUS,EMINUS,GLOBCC
 1003     FORMAT(6X,I5,5G20.10)
        ...
        DO I=-1000,1000
          U(1)  = DFLOAT(I)
          SUM=U(1)+DSQRT(C1)*(U(5)*2.D0/3.D0*C1+
     &         U(6)*2.D0/5.D0*C1**2+
     &         U(7)*2.D0/7.D0*C1**3+
     &         U(8)*2.D0/9.D0*C1**4)
          DO IDATA=1,NDATA
...
C
      ...
      DOUBLE PRECISION N2,SIG2,M2,SIG3,R1,R2,SIG4
      ...
      BWG=N1*(R1/(1.D0+R1)*FOLD(X,GAMMA,SIG1,M1)+
     &     1.D0/(1.D0+R1)*FOLD(X,GAMMA,SIG3,M1))+
     &     N2*(R2/(1.D0+R2)*GAUSS(X,M2,SIG2)+
     &     1.D0/(1.D0+R2)*GAUSS(X,M2,SIG4))+
     &     DSQRT(DM)*

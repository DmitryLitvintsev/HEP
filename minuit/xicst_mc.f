...
C
      ...
      NDATA=1
      OPEN(UNIT=80,FILE='data_points_2027_mc.dat')
...
C*   Open data files
C*
      open(unit=5,file='xicst_mc.data',status='old')
      open(unit=21,file='fcn_mc.dat',status='unknown',form='formatted')
      open(unit=6,file='fit_mc.out',status='unknown',form='formatted')
      ...
      DOUBLE PRECISION XDATA,EXDATA
      COMMON /USER/ XDATA(MXDATA),EXDATA(MXDATA),NDATA
      DOUBLE PRECISION FUN,SUM,SQRT,DEXP,BWG,C1,C2,GAUSS

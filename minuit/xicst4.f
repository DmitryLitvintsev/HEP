...
C*   Open data files
C*
      open(unit=5,file='xicst4.data',status='old')
      open(unit=21,file='fcn4.dat',status='unknown',form='formatted')
      open(unit=22,file='func4.dat',status='unknown',form='formatted')
...
C
      ...
      DOUBLE PRECISION N2,SIG2,M2,RATIO,SIG3
      ...
      BWG=N1*(RATIO/(1.+RATIO)*FOLD(X,GAMMA,SIG1,M1)+
     &    RATIO/(1.+RATIO)*FOLD(X,GAMMA,SIG3,M1))+
     &    N2*GAUSS(X,M2,SIG2)+
     &     DSQRT(DM)*

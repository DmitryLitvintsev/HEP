...
C
      ...
      CALL HROPEN(20,'blah','tst_wrong_sign.hist','N',1024,istat)
      CALL HBOOK1(1,'xicst',700.0,1.4,2.8,0.0)
      CALL HBOOK1(2,'LH',2000.0,-1000.0,1000.0,0.0)


      NDATA=1
      OPEN(UNIT=80,FILE='data_points_unique_ws_min_chi2.dat')
...
C*   Open data files
C*
      open(unit=5,file='xicst3.data',status='old')
      open(unit=21,file='fcn_wrong_sign.dat',
     & status='unknown',form='formatted')
C*      open(unit=6,file='fit_wrong_sign.out',status='unknown',form='formatted')
...
C      CALL HRPUT(0,'tst.hist','N')
 ...
 100  SUM=U(1)+U(9)+DSQRT(C1)*(U(4)*2.D0/3.D0*C1+
     &  U(5)*2.D0/5.D0*C1**2+
     &  U(6)*2.D0/7.D0*C1**3+
     &  U(7)*2.D0/9.D0*C1**4)
C
...
C
      GCM   = (U(3)-1.862D0)/0.002D0
      GCS   = (U(2)-0.008D0)/0.0012D0
      DO I=1,NDATA

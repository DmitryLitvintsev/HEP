C
C this is to run many experiments and check how they look for 
C full blown thing, Xi- pi+
C
      PROGRAM MAIN
      IMPLICIT NONE
...
C     &     status='unknown',form='formatted')
 ...
 1    CONTINUE
      ...
      DO WHILE ( IDATA .LT. NDATA ) 
        MASS = 1.4D0 + RNDM()*(2.8D0-1.4D0)
        PRB  = RNDM()*BWG(MASS0,VSTRT)
